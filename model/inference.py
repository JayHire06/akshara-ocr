...
def recognize(line_images: List[np.ndarray]) -> List[str]:
    ...
    """
    model_path = os.path.join(os.path.dirname(__file__), 'checkpoints', 'best_model.onnx')
    ...
    # Vocab mock logic
    vocab_path = os.path.join(os.path.dirname(__file__), 'config.json')
    if os.path.exists(vocab_path):
        with open(vocab_path, 'r') as f:
            config = json.load(f)
            vocab = config.get('vocab', ['<blank>', 'a', 'b', 'c']) 
    else:
        # Default ASCII range stub
        vocab = ['<blank>'] + [chr(i) for i in range(32, 127)]
